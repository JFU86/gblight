...
this.MooEditable = new Class({
	...
	options: {
		...
		xhtml : true,
		semantics : true,
		actions: 'smiley | bold italic underline | insertunorderedlist insertorderedlist | createlink unlink',
		// actions: 'bold italic underline strikethrough smiley | insertunorderedlist insertorderedlist indent outdent | undo redo | createlink unlink | toggleview',
		handleSubmit: true,
		handleLabel: true,
		disabled: false,
	...
	attach: function(){
		...
		if (Browser.name == 'firefox') this.doc.designMode = 'On';
		...
		(Browser.name == 'ie') ? this.doc.body.contentEditable = true : this.doc.designMode = 'On';
		...
		if (Browser.name == 'firefox' && Browser.version == 2) this.doc.addEvent('focus', function(){
	...
	editorKeyUp: function(e){
		...
		if (this.options.toolbar && (/^enter|left|up|right|down|delete|backspace$/i.test(e.key) || (c >= 33 && c <= 36) || c == 45 || e.meta || e.control)){
			if (Browser.name == 'ie' && Browser.version == 6){ // Delay for less cpu usage when you are typing
	...
	editorKeyDown: function(e){
		...
		if (e.key == 'enter'){
			if (this.options.paragraphise){
				if (e.shift && (Browser.name == 'safari' || Browser.name == 'chrome')){
					...
					e.preventDefault();
				} else if (Browser.name == 'firefox' || Browser.name == 'safari' || Browser.name == 'chrome'){
...
MooEditable.UI.Button = new Class({
	...
	render: function(){
		...
		if (Browser.name == 'ie') this.el.addEvents({
